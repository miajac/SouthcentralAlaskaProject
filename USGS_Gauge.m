...
for dPosition = 1:numel(columnNames)
    ...
    formatFileName = ['load_log_t_usgs_no_title_', sInputChar, '.jpg'];
...
for dPosition = 1:numel(columnNames)
    ...
    plot(mKR3_Date22, y_KR3_2022, 'o--', 'DisplayName', 'K3 Discharge','Color',[0.35,0.70,0.90],'MarkerFaceColor',[0.35,0.70,0.90],'HandleVisibility', 'off','MarkerEdgeColor','k');
    hold on;
    ...
    plot(mMR4_Date22, y_MR4_2022, 'o--', 'DisplayName', 'M4 Discharge','Color',[0.95,0.90,0.25],'MarkerFaceColor',[0.95,0.90,0.25],'HandleVisibility', 'off','MarkerEdgeColor','k');
    ...
    plot(mMC_Date22, y_MC_2022, 'o--', 'DisplayName', 'MC Discharge','Color',[0.90,0.60,0],'MarkerFaceColor',[0.90,0.60,0],'HandleVisibility', 'off', 'MarkerEdgeColor','k');
    ...
    plot(mLS2_Date22, y_LS2_2022, 'o--', 'DisplayName', 'LS2 Discharge','Color',[0,0.60,0.50],'MarkerFaceColor',[0,0.60,0.50],'HandleVisibility', 'off','MarkerEdgeColor','k');
    ...
    hold off;
    sInputChar = char(sInput);
    formatFileName = "conc_t_dis_usgs_%s.svg";
    fileName = sprintf(formatFileName,sInputChar);
    fullFilePath = fullfile(folderName, fileName);
    saveas(gcf, fullFilePath, 'svg');
end
...
for dPosition = 1:numel(columnNames)
    ...
    ylabel(sprintf('%s (%s)', sFormattedInput, yUnit), 'Interpreter', 'tex'); 
    %title(sFormattedInput);
    legend('Location', 'none');
    legend('Position', [0.48 0.5 0.1 0.1], 'Location', 'none');
    grid on;
    ...
    hold off;
    sInputChar = char(sInput);
    formatFileName = "time_series_usgs_no_title_%s.jpg";
    fileName = sprintf(formatFileName,sInputChar);
    fullFilePath = fullfile(folderName, fileName);
    saveas(gcf, fullFilePath, 'jpg');
end
...
%% C-Q Log Transform and Linear Plot with Recession Lines
columnNames = vColumnLabels;
folderName = 'U:/GoA plots/NewPlots';
for dPosition = 1:numel(columnNames)
    sInput = columnNames{dPosition};
    sTitle = ['Element ', sInput];
    figure;
    y_KR3 = mKR3Data(:, dPosition);
    x_KR3 = mKR3Data(:, 1);
    y_KR3 = fillmissing(y_KR3, 'linear');
    plot(x_KR3, y_KR3, 'o', 'DisplayName', 'K3','Color',[0.35,0.70,0.90],'MarkerFaceColor',[0.35,0.70,0.90],'MarkerEdgeColor','k');
    hold on;
    y_MR4 = mMR4Data(:, dPosition);
    x_MR4 = mMR4Data(:, 1);
    y_MR4 = fillmissing(y_MR4, 'linear');
    plot(x_MR4, y_MR4, 'o', 'DisplayName', 'M4','Color',[0.95,0.90,0.25],'MarkerFaceColor',[0.95,0.90,0.25],'MarkerEdgeColor','k');
    y_MC = mMCData(:, dPosition);
    x_MC = mMCData(:, 1);
    y_MC = fillmissing(y_MC, 'linear');
    plot(x_MC, y_MC, 'o', 'DisplayName', 'MC','Color',[0.90,0.60,0],'MarkerFaceColor',[0.90,0.60,0],'MarkerEdgeColor','k');
    y_LS2 = mLS2Data(:, dPosition);
    x_LS2 = mLS2Data(:, 1);
    y_LS2 = fillmissing(y_LS2, 'linear');
    plot(x_LS2, y_LS2, 'o', 'DisplayName', 'LS2','Color',[0,0.60,0.50],'MarkerFaceColor',[0,0.60,0.50],'MarkerEdgeColor','k');
    xlabel('Discharge (m^{3}/s)');  % Label for x-axis
    ylabel('Concentration (mg/L)');  % Label for y-axis
    title(sInput);
    legend('Location', 'eastoutside');
    ...
    hold off;
    sInputChar = char(sInput);
    formatFileName = "conc_dis_log_usgs_%s.jpg";
    fileName = sprintf(formatFileName,sInputChar);
    fullFilePath = fullfile(folderName, fileName);
    saveas(gcf, fullFilePath, 'jpg');
end