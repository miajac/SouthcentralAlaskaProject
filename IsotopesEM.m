...
M_Glacier = mMatFull(vM_EM == "Glacier",:);
M_Precip = mMatFull(vM_EM == "Precip",:);
...
%scatter(M_Precip(:,1), M_Precip(:,2), 'k*','DisplayName', 'Precipitation','SizeData',markerSizeS, 'HandleVisibility', 'on');
scatter(M_Glacier(:,1), M_Glacier(:,2), 'k^','MarkerFaceColor',[0.616, 0.765, 0.902],'DisplayName', 'Glacial*','SizeData',markerSizeG, 'HandleVisibility', 'on');

ax = gca;
ax.FontSize = 10; 
title('Matanuska','FontSize',18);
legend('Location', 'SouthEast','FontSize',legendFontSize);
grid off;
hold off;
...
ax = gca;
ax.FontSize = 10; 
title('Little Susitna','FontSize',18);
legend('Location', 'NorthWest','FontSize',legendFontSize);
grid off;
hold off;
...
scatter(CS(:, 1), CS(:, 2), 'k^', 'MarkerFaceColor', [0.616, 0.765, 0.902],'DisplayName', 'Subglacial', 'SizeData', markerSizeD,'HandleVisibility','off');
xlabel('δ¹⁸O (‰)');  
ax = gca;
ax.FontSize = 10; 
title('Delta Range','FontSize',18);
legend('Location', 'SouthEast','FontSize',legendFontSize);
grid off;
hold off;
